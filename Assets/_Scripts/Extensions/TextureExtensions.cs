...
namespace TS.Extensions
{
    public static class TextureExtensions
    {
        ...
        public static void Scale(this Texture2D texture, int width, int height, TextureScaleMode scaleMode = TextureScaleMode.Bilinear)
        {
            switch (scaleMode)
            {
        ...
        public static Vector2 GetSize(this Texture2D texture, int max)
        {
            Vector2 result = new Vector2(texture.width, texture.height);

            float ratio = result.x / result.y;

            if (result.x > result.y && result.x > max)
            {
                result.x = max;
                result.y = Mathf.RoundToInt(result.x / ratio);
                return result;
            }

            if (result.y > result.x && result.y > max)
            {
                result.y = max;
                result.x = Mathf.RoundToInt(result.y * ratio);
                return result;
            }
